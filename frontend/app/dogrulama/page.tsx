...
export default function DogrulamaPage() {
  ...
  return (
    <div className="h-screen overflow-auto p-5 pt-14 md:p-8 md:pt-8 space-y-6">
      {toast && (
        <div role="alert" aria-live="polite" className="fixed top-4 right-4 z-50 px-4 py-2 bg-[#3DD68C]/20 border border-[#3DD68C]/30 text-[#3DD68C] text-[15px] rounded-lg animate-fade-in">{toast}</div>
      )}

      <div>
        <h1 className="text-[20px] font-bold tracking-tight text-[#ECECEE]">Atif Dogrulama</h1>
        <p className="text-[14px] text-[#5C5C5F] mt-1">Hukuki metinlerdeki atıf ve referansları doğrulayın</p>
      </div>
      ...
      <div className="space-y-3">
        <textarea value={text} onChange={(e) => setText(e.target.value)}
          ...
          rows={12}
          className="w-full bg-[#111113] border border-white/[0.06] rounded-2xl px-4 py-3 text-[16px] font-mono text-[#ECECEE] placeholder:text-[#5C5C5F] placeholder:font-sans focus:outline-none focus:border-[#6C6CFF]/40 focus:bg-[#16161A] transition-all duration-200 resize-y leading-relaxed" />
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            {text.length > 0 && (<span className="text-[13px] text-[#5C5C5F]">{text.length} karakter</span>)}
            {!text && (
              <button onClick={handleLoadExample}
                className="px-2.5 py-1.5 text-[14px] text-[#8B8B8E] hover:text-[#ECECEE] border border-white/[0.06] hover:border-white/[0.10] rounded-lg transition-all duration-150">Örnek Metin Yükle</button>
            )}
            {text.length > 0 && (
              <button onClick={handleClear}
                className="px-2.5 py-1.5 text-[14px] text-[#8B8B8E] hover:text-[#ECECEE] border border-white/[0.06] hover:border-white/[0.10] rounded-lg transition-all duration-150">Temizle</button>
            )}
          </div>
          <button onClick={handleVerify} disabled={loading || !text.trim()}
            className="px-5 py-2 bg-[#6C6CFF] hover:bg-[#7B7BFF] disabled:bg-[#1A1A1F] disabled:text-[#5C5C5F] rounded-lg text-[15px] font-medium text-white transition-colors duration-150">
            {loading ? (
              <div className="flex items-center gap-2">
      ...
      {error && (
        <div className="bg-[#E5484D]/10 border border-[#E5484D]/20 rounded-xl p-3 text-[15px] text-[#E5484D]">
          {error}
          <button onClick={handleVerify} className="block mt-2 text-[14px] text-[#E5484D]/80 hover:text-[#E5484D] underline underline-offset-2 transition-colors">Tekrar Dene</button>
        </div>
      )}
      ...
      {results && (
        <motion.div className="space-y-5" variants={listContainer} initial="hidden" animate="show">
          ...
          <div className="space-y-2">
            <h2 className="text-[15px] font-medium text-[#8B8B8E]">Atıf Detayları ({results.details.length})</h2>
      ...
      {!loading && !results && !error && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-[15px] text-[#8B8B8E]">Doğrulanacak metni yukarıdaki alana yapıştırın</p>
          <p className="text-[14px] text-[#5C5C5F] mt-1 max-w-md">Sistem içtihat numaraları, kanun maddeleri ve diğer hukuki referansları otomatik olarak tespit edecektir</p>
        </div>
      )}
    </div>
  );
