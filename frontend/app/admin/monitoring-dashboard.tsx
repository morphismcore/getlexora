...
export default function MonitoringDashboard({ token, apiUrl }: { token: string | null; apiUrl: string }) {
  ...
  if (!data) {
    return <div className="text-[15px] text-[#E5484D] text-center py-12">Monitoring verileri alinamadi.</div>;
  }
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={fetchMonitoring} className="text-[13px] text-[#6C6CFF] hover:text-[#5B5BEE] transition-colors">Yenile</button>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <div className="bg-[#111113] border border-white/[0.06] rounded-xl p-4">
          <p className="text-[22px] font-semibold text-[#ECECEE]">{formatUptime(data.uptime_seconds)}</p>
          <p className="text-[13px] text-[#5C5C5F] mt-0.5">Uptime</p>
        </div>

        {/* CPU */}
        <div className="bg-[#111113] border border-white/[0.06] rounded-xl p-4">
          <p className="text-[22px] font-semibold" style={{ color: cpuColor(data.cpu_percent) }}>
            %{data.cpu_percent.toFixed(1)}
          </p>
          <p className="text-[13px] text-[#5C5C5F] mt-0.5">CPU</p>
        ...
        {/* RAM */}
        <div className="bg-[#111113] border border-white/[0.06] rounded-xl p-4">
          <p className="text-[22px] font-semibold text-[#A78BFA]">{data.memory_usage_mb.toLocaleString("tr-TR")}</p>
          <p className="text-[13px] text-[#5C5C5F] mt-0.5">RAM (MB)</p>
          <div className="w-full h-1.5 bg-[#1A1A1F] rounded-full mt-2 overflow-hidden">
        ...
        {/* Disk */}
        <div className="bg-[#111113] border border-white/[0.06] rounded-xl p-4">
          <p className="text-[22px] font-semibold" style={{ color: data.disk_usage_pct >= 90 ? "#E5484D" : data.disk_usage_pct >= 75 ? "#FFB224" : "#3DD68C" }}>
            %{data.disk_usage_pct.toFixed(1)}
          </p>
          <p className="text-[13px] text-[#5C5C5F] mt-0.5">Disk</p>
          <div className="w-full h-1.5 bg-[#1A1A1F] rounded-full mt-2 overflow-hidden">
        ...
        {/* Req/min */}
        <div className="bg-[#111113] border border-white/[0.06] rounded-xl p-4">
          <p className="text-[22px] font-semibold text-[#6C6CFF]">{data.requests_per_minute}</p>
          <p className="text-[13px] text-[#5C5C5F] mt-0.5">Istek/dk</p>
          <p className="text-[12px] text-[#5C5C5F] mt-1">{data.requests_total.toLocaleString("tr-TR")} toplam</p>
        </div>

        {/* Error rate */}
        <div className="bg-[#111113] border border-white/[0.06] rounded-xl p-4">
          <p className="text-[22px] font-semibold" style={{ color: data.error_rate_pct > 5 ? "#E5484D" : data.error_rate_pct > 1 ? "#FFB224" : "#3DD68C" }}>
            %{data.error_rate_pct.toFixed(2)}
          </p>
          <p className="text-[13px] text-[#5C5C5F] mt-0.5">Hata Orani</p>
          <p className="text-[12px] text-[#5C5C5F] mt-1">{data.avg_response_time_ms.toFixed(1)}ms ort.</p>
        </div>
      </div>

      {/* Service Status Grid */}
      <div className="bg-[#111113] border border-white/[0.06] rounded-xl p-5">
        <h3 className="text-[15px] font-semibold text-[#ECECEE] mb-4">Servis Durumu</h3>
          ...
          {Object.entries(data.services).map(([key, svc]) => (
            <div key={key} className="bg-[#09090B] rounded-lg p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[15px] font-medium text-[#ECECEE]">{SERVICE_LABELS[key] || key}</span>
              </div>
              <div className="space-y-1">
                <div className="flex items-center justify-between">
                  <span className="text-[13px] text-[#5C5C5F]">Yanit suresi</span>
                  <span className="text-[13px] font-mono text-[#8B8B8E]">{svc.response_ms}ms</span>
                </div>
                {svc.memory_mb !== undefined && (
                  <div className="flex items-center justify-between">
                    <span className="text-[13px] text-[#5C5C5F]">Bellek</span>
                    <span className="text-[13px] font-mono text-[#8B8B8E]">{svc.memory_mb}MB</span>
                  </div>
                )}
                {svc.error && (
                  <p className="text-[12px] text-[#E5484D] mt-1 truncate" title={svc.error}>{svc.error}</p>
                )}
              </div>
              <div className="mt-2 pt-2 border-t border-white/[0.04]">
                <span className={`px-2 py-0.5 rounded text-[12px] font-medium ${svc.status === "ok" ? "bg-[#3DD68C]/10 text-[#3DD68C]" : "bg-[#E5484D]/10 text-[#E5484D]"}`}>
      ...
      <div className="bg-[#111113] border border-white/[0.06] rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-[15px] font-semibold text-[#ECECEE]">Embedding Istatistikleri</h3>
          <div className="text-right">
            <p className="text-[26px] font-bold text-[#6C6CFF]">{data.ingestion.total_embeddings.toLocaleString("tr-TR")}</p>
            <p className="text-[12px] text-[#5C5C5F]">Toplam Embedding</p>
          </div>
        </div>
          ...
          {embeddingSources.map((src) => {
            ...
            return (
              <div key={src.key} className="flex items-center gap-3">
                <span className="text-[14px] text-[#8B8B8E] w-20 shrink-0">{src.label}</span>
                ...
                <span className="text-[14px] font-mono text-[#ECECEE] w-16 text-right shrink-0">{count.toLocaleString("tr-TR")}</span>
              </div>
            );
          })}
        </div>
        ...
        <div className="flex items-center gap-6 pt-3 border-t border-white/[0.06]">
          <div>
            <p className="text-[13px] text-[#5C5C5F]">Son Ingestion</p>
          ...
          <div>
            <p className="text-[13px] text-[#5C5C5F]">Gunluk Yeni</p>
            <p className="text-[14px] text-[#3DD68C] font-medium">+{data.ingestion.daily_new_count}</p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="bg-[#111113] border border-white/[0.06] rounded-xl p-4">
      <div className="flex items-center justify-between mb-2">
        <span className="text-[14px] font-medium text-[#ECECEE]">{title}</span>
        <span className="text-[14px] font-mono" style={{ color }}>{formatValue(latestVal)}</span>
      </div>
      ...
      <div className="flex items-center justify-between mt-1">
        <span className="text-[12px] text-[#5C5C5F]">{formatValue(minVal)}</span>
        <span className="text-[12px] text-[#5C5C5F]">{formatValue(maxVal)}</span>
      </div>
    </div>
  );
}
