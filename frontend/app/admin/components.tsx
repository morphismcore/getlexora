...
export function ConfirmDialog({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center">
      ...
      >
        <h3 className="text-[17px] font-semibold text-[#ECECEE] mb-2">{title}</h3>
        <p className="text-[15px] text-[#8B8B8E] mb-6">{message}</p>
        <div className="flex gap-3 justify-end">
          <button
            onClick={onCancel}
            className="px-4 py-2 text-[14px] font-medium text-[#8B8B8E] hover:text-[#ECECEE] bg-white/[0.04] hover:bg-white/[0.08] rounded-lg transition-colors"
          >
            Iptal
          </button>
          <button
        ...
        <>
          ...
          >
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.06]">
              <h3 className="text-[17px] font-semibold text-[#ECECEE]">{title}</h3>
              <button
                onClick={onClose}
...
export function EmptyState({ icon, title, description, action }: { icon: string; title: string; description: string; action?: React.ReactNode }) {
  return (
    ...
    >
      ...
      <h3 className="text-[16px] font-medium text-[#ECECEE] mb-1">{title}</h3>
      <p className="text-[14px] text-[#5C5C5F] max-w-xs">{description}</p>
...
export function FormField({ label, children, hint }: { label: string; children: React.ReactNode; hint?: string }) {
  return (
    <div>
      <label className="text-[14px] font-medium text-[#8B8B8E] block mb-1.5">{label}</label>
      {children}
      {hint && <p className="text-[12px] text-[#5C5C5F] mt-1">{hint}</p>}
    </div>
  );
}
