...
import {
  type AnalyzeResult, type UploadHistoryItem,
  DOC_TYPE_LABELS, PATTERN_LABELS, PATTERN_COLORS,
  getDocTypeStyle, detectRisks,
} from "./_components/types";
import { FormatIcon, EntityCard, UploadProgressBar } from "./_components/ui-parts";
...
export default function BelgePage() {
  ...
  const uploadFile = useCallback(async (file: File) => {
    ...
    if (!validTypes.includes(file.type) && ext !== "pdf" && ext !== "docx") { setError("Sadece PDF ve DOCX dosyaları kabul edilir."); return; }
    ...
    try {
      const data = await new Promise<AnalyzeResult>((resolve, reject) => {
        ...
        xhr.onload = () => {
          ...
          if (xhr.status >= 200 && xhr.status < 300) {
            try { resolve(JSON.parse(xhr.responseText)); } catch { reject(new Error("Yanıt ayrıştırılamadı")); }
          } else {
            try { const errData = JSON.parse(xhr.responseText); reject(new Error(errData?.detail || `Hata: ${xhr.status}`)); } catch { reject(new Error(`Hata: ${xhr.status}`)); }
          }
        };
      ...
      setTimeout(() => {
        setResult(data); setActiveTab("overview"); setToast("Belge başarıyla analiz edildi");
        const historyItem: UploadHistoryItem = {
          file_name: data.file_name, file_type: data.file_type, document_type: data.document_type,
          text_length: data.text_length, citations_count: data.citations.length, timestamp: new Date().toISOString(),
        };
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {!result && activeTab !== "history" && (
          <motion.div
            ...
            animate={dragOver ? { scale: 1.01 } : { scale: 1 }} transition={{ type: "spring", stiffness: 300, damping: 20 }}
            className={`relative flex flex-col items-center justify-center gap-4 rounded-2xl border-2 border-dashed cursor-pointer transition-all duration-300 py-14 px-6 ${
              dragOver ? "border-[#6C6CFF] bg-[#6C6CFF]/[0.06] shadow-[0_0_30px_rgba(108,108,255,0.1)]" : "border-[#2A2A2E] bg-[#111113] hover:border-[#3A3A3E] hover:bg-[#151517]"
            }`}>
